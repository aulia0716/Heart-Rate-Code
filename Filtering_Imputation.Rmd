...
impute_rr_intervals <- function(df) {
  ...
  while (i < nrow(df)) {
    ...
    while (!is.na(df$Time[i]) && !is.na(df$Time[i + 1]) && (df$Time[i + 1] - df$Time[i]) > 1.3) {
      ...
      repeat {
        if (i - 9 <= 0) {
          # Use as many previous RR intervals as available
          mu <- mean(df$RR[1:i])
          sigma <- sd(df$RR[1:i])
        } else {
          # Use mean and std deviation from 10 previous observation
          mu <- mean(df$RR[(i - 9):i])
...
2. The mean and standard deviation from the ten first observation issue --> I use as many previous RR intervals as are available, ensuring the function continues to execute correctly for all iterations. We can edit it later if we have other solutions. 
