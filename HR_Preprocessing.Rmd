...
for (i in 1:(nrow(df)-1)) {
  ...
  if (!is.na(df$RR[i]) && df$RR[i] < 0.3){
    if (i+1 > nrow(df)) break
      ...
      E_tot_r <- if (!is.na(E_r) && !is.na(E_l)) ER_r + ER_l else NA
...
- The filtered dataset shows the minimum RR of 304.07. It means that there is not any artifacts with value < 0.3 s anymore. 

- However, the here are 37 observations that still not filtered --> Need to debugging the code again. 

- Check the data result for the 37 observation only. 

```{r}
artifacts_original <- subset(data_original, RR < 300)
dim(artifacts_original)
```

```{r}
filtered_out_ids <- data_original %>%
  filter(!No %in% data_filtered$No) %>%
  select(No)
dim(filtered_out_ids)
```

```{r}
artifacts_missing <- artifacts_original %>%
  filter(!No %in% filtered_out_ids$No) %>%
  select(No)
dim(artifacts_missing) #why 38 not 37?
```
```{r}
# Ensure both are data.tables
setDT(filtered_out_ids)
setDT(artifacts_missing)

# Identify duplicated IDs
duplicated_ids <- filtered_out_ids[No %in% artifacts_missing$No, No]

# Check if there are any duplicated IDs and print the result
if (length(duplicated_ids) > 0) {
  cat("Duplicated IDs:\n")
  print(duplicated_ids)
} else {
  cat("No duplicated IDs found.")
}
```

```{r}
# Assuming data1 and data2 are your data frames and ID is the column name for the IDs

# Extracting IDs from both datasets
ids_data1 <- data_original$No
ids_data2 <- data_filtered$No

# Finding the IDs that are in data1 but not in data2
missing_ids <- setdiff(ids_data1, ids_data2)

# Checking the dimensions
dim_data1 <- length(ids_data1)
dim_data2 <- length(ids_data2)
dim_missing <- length(missing_ids)

# Printing the dimensions
cat("Dimension of data1 IDs:", dim_data1, "\n")
cat("Dimension of data2 IDs:", dim_data2, "\n")
cat("Dimension of missing IDs:", dim_missing, "\n")

# Printing the missing IDs
cat("Missing IDs:\n")
print(missing_ids)

# Check if there are duplicate IDs in data1
duplicate_ids_data1 <- ids_data1[duplicated(ids_data1)]
if(length(duplicate_ids_data1) > 0) {
  cat("Duplicate IDs in data1:\n")
  print(unique(duplicate_ids_data1))
} else {
  cat("No duplicate IDs in data1.\n")
}

# Check if there are duplicate IDs in data2
duplicate_ids_data2 <- ids_data2[duplicated(ids_data2)]
if(length(duplicate_ids_data2) > 0) {
  cat("Duplicate IDs in data2:\n")
  print(unique(duplicate_ids_data2))
} else {
  cat("No duplicate IDs in data2.\n")
}
...
# Extracting IDs from both datasets
ids_data1 <- artifacts_original$No
ids_data2 <- filtered_out_ids$No
...
# Extract rows based on the list of IDs
extracted_df_ori <- data_original[No %in% artifacts_missing$No]

# Display dimensions of the extracted data frame
dim(extracted_df_ori)
```
...
# Extract rows based on the list of IDs
extracted_df_filt <- data_filtered[No %in% artifacts_missing$No]

# Display dimensions of the extracted data frame
dim(extracted_df_filt)
```

```{r}
View(extracted_df_filt)
```

### Next : I need to check the data condition for other participant, especially the real participant. 


### Next step 

- Clarify the naming format of folder and file, edit the code especially about accessing the file. 

- Finish the filtering. 


### Side note --> This is unused code, but may need to use that in other time, so I keep it. 
